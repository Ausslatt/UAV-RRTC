...
OBSTACLES = [
    # bitstar walls
    ((8.0- X_PAD, 40.0),      (-15.5- Y_PAD, -14.5),   (-2.0, 2.0)),   # bitstar_wall_0
    ((-20.0- X_PAD, -3.0),    (-11.0- Y_PAD, -5.0),    (-2.0, 5.0)),   # bitstar_wall_1
    ((3.5 - X_PAD, 6.5 + X_PAD),       (-40.0 - Y_PAD, -5.0 + Y_PAD),    (0 - Z_PAD, 5.0 + Z_PAD)),   # bitstar_wall_2

    # bitstar towers
    ((-8.5 - X_PAD, -5.5 + X_PAD),     (3.5 - Y_PAD, 6.5 + Y_PAD),       (-6.0 - Z_PAD, 6.0 + Z_PAD)),   # bitstar_tower_0
    ((-3.0 - X_PAD, 0.0 + X_PAD),      (0.5 - Y_PAD, 3.5 + Y_PAD),       (-7.5 - Z_PAD, 7.5 + Z_PAD)),   # bitstar_tower_1
    ((9.5 - X_PAD, 12.5 + X_PAD),      (1.5 - Y_PAD 4.5 + Y_PAD),       (-3.5 - Z_PAD, 3.5 + Z_PAD)),   # bitstar_tower_2
    ((19.5 - X_PAD, 22.5 + X_PAD),     (1.5 - Y_PAD, 4.5 + Y_PAD),       (-3.5 - Z_PAD, 3.5 + Z_PAD)),   # bitstar_tower_3
    ((13.5 - X_PAD, 16.5 + X_PAD),     (9.5 - Y_PAD, 12.5 + Y_PAD),      (0.0 - Z_PAD, 5.0 + Z_PAD)),   # bitstar_tower_4 
    ((20.5 - X_PAD, 23.5 + X_PAD),     (13.5 - Y_PAD, 16.5 + Y_PAD),     (0.0 - Z_PAD, 10.0 + Z_PAD)), # bitstar_tower_7
...
# Sampling bounds for the RRT (adjust to match your world as needed)
SAMPLE_BOUNDS = {
    'min_x': -20.0,
    'max_x':  40.0,
    'min_y': -40.0,
...
class Vertex(object):
    def __init__(self, x, y, z, yaw, parent=None):
        """
        Simple vertex/state: position + yaw + parent pointer.
        """
        self.x = x
        self.y = y
        self.z = z
...
class RRTTree(object):
    """
    Simple tree for RRT-Connect: root + list of vertices.
    """
...
class RRTConnectPlanner(object):
    ...
    Uses:
      - Bi-directional trees (start and goal)
      - Straight-line steering with fixed step size
      - Uniform random sampling within SAMPLE_BOUNDS
      - Simple goal bias
    """
...
class BitStarPlannerNode(object):
    """
    ROS node wrapper (keeps original name for compatibility) that now runs RRT-Connect.
    """

    def __init__(self):
        rospy.init_node('bitstar_planner')
    ...
    def densify_path(self, path, max_seg_len=1.0):
        """
        Take a path [(x,y,z,yaw), ...] and insert intermediate points so that
        no segment is longer than max_seg_len [m]. Keep a constant yaw
        equal to the start yaw so we don't slam the yaw controller.
        """
    ...
    def path_to_trajectory(self, path):
        ...
        # 2) Add a little hover at the goal so it can settle
        if traj.points:
            last = traj.points[-1]
